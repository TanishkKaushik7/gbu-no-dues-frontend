...
const AdminDashboard = () => {
  ...
  const adminTabPaths = {
    ...
    application_logs: "/admin/application_logs",
    system_logs: "/admin/system_logs",
  ...
  const getTabFromPath = (pathname) => {
    ...
    if (segment === "admin" || segment === "dashboard" || !segment) {
      return "dashboard";
    }
  ...
  if (!sidebarItems.find((item) => item.id === "system_logs")) {
    ...
    const appLogsIndex = sidebarItems.findIndex(
      (item) => item.id === "application_logs",
    );
    if (appLogsIndex !== -1)
      sidebarItems.splice(appLogsIndex + 1, 0, sysLogsItem);
  ...
  if (!sidebarItems.find((item) => item.id === "metrics")) {
    ...
    const reportsIndex = sidebarItems.findIndex(
      (item) => item.id === "reports",
    );
    if (reportsIndex !== -1)
      sidebarItems.splice(reportsIndex + 1, 0, metricsItem);
  ...
  useEffect(() => {
    if (
      user &&
      !["admin", "super_admin", "dean", "hod", "registrar"].includes(user.role)
    ) {
  ...
  useEffect(() => {
    const handleResize = () => {
      // Keep desktop navigation accessible after switching from mobile viewport.
  ...
  const handleSelectApplication = async (item) => {
    ...
    try {
      ...
      const res = await authFetch(
        `/api/applications/status?search_query=${encodeURIComponent(q)}`,
      );
      if (res.ok) {
        const data = await res.json();
        if (data.application) {
          ...
          alert("No active No-Dues application found.");
        }
      }
    } catch (err) {
  ...
  const navigateToTab = (tabId) => {
    const targetPath = adminTabPaths[tabId] || `/admin/${tabId}`;
    setActiveTab(tabId);
    navigate(targetPath);
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid gap-6 lg:grid-cols-7">
              ...
              <div className="lg:col-span-4">
                <QuickActions
                  onRegisterUser={() =>
                    setModalOpen({ ...modalOpen, register: true })
                  }
              ...
              <div className="lg:col-span-3 bg-white rounded-[2.5rem] border border-slate-200 p-8 shadow-sm">
                <RecentLogsWidget
                  onNavigate={() => navigateToTab("application_logs")}
                />
              </div>
            </div>
            ...
            <div className="w-full bg-white rounded-[2.5rem] border border-slate-200 p-8 shadow-sm">
              <div className="mb-6">
                ...
                <p className="text-[10px] font-bold text-slate-400  tracking-widest">
                  Live traffic & processing trends
                </p>
              </div>
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-900 overflow-hidden h-screen">
      <header className="h-16 sm:h-20 bg-[#1e40af] text-white flex items-center justify-between px-4 sm:px-8 shadow-xl z-30 sticky top-0 shrink-0 border-b border-blue-800/50">
        <div className="flex items-center gap-4 sm:gap-6">
          <button
            className="md:hidden p-2 hover:bg-blue-700 rounded-xl z-20 relative"
            onClick={() => setIsSidebarOpen(!isSidebarOpen)}
          >
            <Menu className="h-6 w-6" />
          </button>
          <div className="flex flex-col">
            <h1 className="text-lg sm:text-xl font-black tracking-tight cursor-default select-none">
              GBU No Dues
            </h1>
            <span className="text-[8px] sm:text-[9px] font-black text-blue-300  tracking-[0.3em]">
              Management Console
            </span>
          </div>
        </div>
        ...
        >
          <div className="relative group">
            {isSearching ? (
              <Loader2 className="absolute left-4 top-2.5 h-5 w-5 text-blue-400 animate-spin z-10" />
            ) : (
          ...
          <AnimatePresence>
            {showSearchResults && (
              <motion.div
                ...
                exit={{ opacity: 0, y: 10 }}
                className="absolute top-14 left-0 right-0 bg-white border border-slate-200 shadow-2xl rounded-[1.5rem] overflow-hidden z-[100] p-4"
              >
                <div className="max-h-[400px] overflow-y-auto space-y-1 custom-scrollbar">
                  ...
                  searchResults?.applications?.length === 0 ? (
                    ...
                    <>
                      {searchResults?.students?.length > 0 && (
                        <div className="mb-2">
                          <div className="text-[10px] font-black text-slate-400  tracking-widest px-3 py-1 bg-slate-50 rounded-lg mb-1">
                            Students
                          </div>
                        ...
                        <div>
                          ...
                          {searchResults.applications.map((app, idx) => (
                            ...
                            >
                              <div className="flex flex-col">
                                <span className="text-sm font-bold text-slate-800 group-hover:text-blue-700">
                                  {app.display_id || app.id}
                                </span>
                                <span className="text-xs text-slate-500 font-mono">
                                  {app.roll_number || app.student_roll}
                                </span>
                              </div>
                              <span
                                className={`text-[9px] font-bold px-2 py-1 rounded-md  tracking-wider ${app.status === "cleared" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
                              >
                                {app.status || "Active"}
                              </span>
                            </button>
        ...
        <div className="flex items-center gap-4">
          <div className="relative" ref={settingsRef}>
            <button
              ...
              className="h-10 w-10 flex items-center justify-center rounded-2xl bg-white/10 hover:bg-white/20 transition-all active:scale-90"
            >
              <Settings
                className={`h-5 w-5 ${isSettingsOpen ? "rotate-90" : ""} transition-transform duration-500`}
              />
            </button>
            <AnimatePresence>
              {isSettingsOpen && (
                <motion.div
                  ...
                  className="absolute right-0 mt-4 w-64 bg-white border border-slate-200 shadow-2xl rounded-[1.5rem] overflow-hidden z-50 p-3"
                >
                  <div className="px-4 py-3 mb-2 bg-slate-50 rounded-2xl border border-slate-100">
                    <p className="text-sm font-bold text-slate-800 truncate">
                      {user?.name}
                    </p>
                    <p className="text-[9px] font-black text-blue-600  mt-1">
                      {user?.role}
                    </p>
                  </div>
                  <button
                    ...
                    className="w-full flex items-center gap-3 p-3 text-slate-600 hover:bg-slate-50 rounded-xl transition-colors font-bold text-[10px]  tracking-widest"
                  >
                    <User size={16} className="text-blue-500" /> Admin Profile
                  </button>
                  <button
                    onClick={() => {
                      setModalOpen({ ...modalOpen, logout: true });
                      setIsSettingsOpen(false);
                    }}
                    className="w-full flex items-center gap-3 p-3 text-red-500 hover:bg-red-50 rounded-xl transition-colors font-bold text-[10px] tracking-widest"
                  >
          ...
          <div className="h-10 w-10 rounded-2xl bg-white text-blue-700 flex items-center justify-center font-black text-xs shadow-lg uppercase border-2 border-blue-400/20">
      ...
      <div className="flex flex-1 overflow-hidden">
        <aside
          className={`${isSidebarOpen ? "w-72" : "w-0 opacity-0 overflow-hidden pointer-events-none"} md:w-72 md:opacity-100 md:overflow-visible md:pointer-events-auto bg-white border-r border-slate-200 transition-all duration-300 flex flex-col z-40 fixed md:relative h-[100dvh] md:h-auto`}
        >
          <div className="flex-1 overflow-y-auto py-6 px-4 space-y-1.5 custom-scrollbar">
            {sidebarItems.map((item) => (
              <button
                ...
                className={`w-full flex items-center h-11 px-4 rounded-2xl text-[10px] uppercase tracking-[0.15em] font-black transition-all group ${activeTab === item.id ? "bg-blue-600 text-white shadow-lg shadow-blue-200" : "text-slate-400 hover:bg-slate-50 hover:text-slate-800"}`}
              >
                <item.icon
                  className={`mr-4 h-4 w-4 ${activeTab === item.id ? "text-white" : "text-slate-300 group-hover:text-blue-500"} transition-transform group-hover:scale-110`}
                />
                <span>{item.label}</span>
              </button>
            ))}
          </div>
          <div className="p-4 pb-[max(1rem,env(safe-area-inset-bottom))] border-t border-slate-100 bg-slate-50/50 shrink-0">
            <button
              onClick={() => setModalOpen({ ...modalOpen, logout: true })}
              className="w-full flex items-center h-11 px-4 rounded-xl text-[10px] font-black uppercase tracking-[0.15em] text-red-500 bg-red-50 hover:bg-red-100 transition-all group border border-red-100"
            >
