...
const MetricsManagement = () => {
  ...
  const fetchAllMetrics = useCallback(() => {
    ...
    // 1. Health
    authFetch("/api/metrics/health/details")
  ...
  const handleClearCache = async (scope) => {
    ...
    try {
      const res = await authFetch(`/api/metrics/clear-cache?scope=${scope}`, {
        method: "POST",
      });
      if (res.ok) {
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-gray-500 mt-0.5">
                Live monitoring & infrastructure health
              </p>
            </div>
          </div>
        ...
        >
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-xl transition-colors">
                  ...
                  <span
                    className={`text-xs px-2.5 py-1 rounded-full font-semibold ${getStatusPillClass(health?.database?.status)}`}
                  >
                ...
                <div className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-xl transition-colors">
                  ...
                  <span
                    className={`text-sm font-semibold ${health?.database?.latency_ms > 300 ? "text-red-600" : "text-green-600"}`}
                  >
                ...
                <div className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-xl transition-colors">
                  <div className="flex items-center gap-3 text-sm font-medium text-gray-700">
                    <Activity className="h-4 w-4 text-purple-500" /> Redis
                    Latency
                  </div>
                  <span
                    className={`text-sm font-semibold ${health?.redis?.latency_ms > 10 ? "text-red-600" : "text-green-600"}`}
                  >
                ...
                <div className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-xl transition-colors">
                  ...
                  <span
                    className={`text-xs px-2.5 py-1 rounded-full font-semibold ${getStatusPillClass(health?.smtp?.status)}`}
                  >
                    {formatStatus(health?.smtp?.status)}
                  </span>
                </div>
            ...
            >
              ...
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-xs font-bold text-gray-400 flex items-center gap-2 uppercase tracking-wider">
                  <HardDrive className="h-4 w-4 text-purple-500" /> Redis
                  Storage
                </h3>
                <button
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
                    ...
                    <p className="text-2xl font-semibold text-gray-900">
                      {redisStats?.metrics?.memory?.used || "--"}
                    </p>
                  </div>
                  <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
                    <p className="text-xs font-medium text-gray-500 mb-1">
          ...
          <div className="space-y-6 lg:col-span-2">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 relative">
              {!dashStats && (
                <div className="absolute inset-0 bg-gray-50/50 backdrop-blur-sm z-10 flex items-center justify-center rounded-2xl">
                  <Loader2 className="animate-spin text-blue-500" />
                </div>
              )}
              ...
              {[
                {
                  label: "Total Apps",
                  value: dashStats?.metrics?.total_applications || 0,
                  color: "text-blue-600",
                  bg: "bg-blue-50",
                },
                {
                  label: "Completed",
                  value: dashStats?.metrics?.completed || 0,
                  color: "text-green-600",
                  bg: "bg-green-50",
                },
                {
                  label: "Pending",
                  value: dashStats?.metrics?.pending || 0,
                  color: "text-amber-600",
                  bg: "bg-amber-50",
                },
                {
                  label: "Rejected",
                  value: dashStats?.metrics?.rejected || 0,
                  color: "text-red-600",
                  bg: "bg-red-50",
                },
              ].map((stat, idx) => (
                ...
                >
                  <div
                    className={`h-10 w-10 rounded-full ${stat.bg} ${stat.color} flex items-center justify-center mb-3`}
                  >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <motion.div
                variants={itemVariants}
                className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm flex flex-col relative"
              >
                {!dashStats && (
                  <div className="absolute inset-0 bg-white/50 backdrop-blur-sm z-10 flex items-center justify-center rounded-2xl">
              ...
              >
                ...
                <h3 className="text-xs font-bold text-gray-400 mb-5 flex items-center gap-2 uppercase tracking-wider">
                  <Activity className="h-4 w-4 text-green-500" /> Top API
                  Traffic
                </h3>
                        ...
                        <div key={i} className="space-y-1.5">
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-gray-600 truncate pr-2 font-medium flex items-center gap-2">
                              <span
                                className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${t.method === "GET" ? "bg-blue-50 text-blue-600" : "bg-green-50 text-green-600"}`}
                              >
                                {t.method}
                              </span>
