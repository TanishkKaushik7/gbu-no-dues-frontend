...
import { 
  ...
  Landmark, Edit3, GraduationCap, ChevronDown, Check 
} from 'lucide-react';
...
const ShadcnSelect = ({ value, onChange, options, placeholder, disabled, loading, icon: Icon, theme = 'slate', textClass }) => {
  ...
  const themes = {
    slate: "bg-slate-50 border-slate-200 focus:border-blue-500 focus:ring-blue-500/10 hover:bg-slate-100",
    blue: "bg-blue-50/30 border-blue-100 focus:border-blue-500 focus:ring-blue-500/10 hover:bg-blue-50",
    emerald: "bg-emerald-50/30 border-emerald-100 focus:border-emerald-500 focus:ring-emerald-500/10 hover:bg-emerald-50"
  };
    ...
    <div className="relative w-full" ref={ref}>
      ...
      <div 
        ...
        className={cn(
          ...
          disabled || loading ? "opacity-60 cursor-not-allowed" : "cursor-pointer focus:bg-white focus:ring-4",
          themes[theme],
          isOpen ? "bg-white ring-4" : ""
        )}
      >
        <div className="flex items-center gap-3 truncate">
          {loading ? <Loader2 className={cn("animate-spin w-4 h-4 shrink-0", iconColors[theme])} /> : Icon ? <Icon className={cn("w-4 h-4 shrink-0", iconColors[theme])} /> : null}
          <span className={cn("truncate", !selectedOption && "text-slate-400 font-normal", textClass || "text-sm font-bold")}>
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-5 max-h-[70vh] overflow-y-auto custom-scrollbar">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <ShadcnSelect
                  value={formData.role}
                  onChange={(val) => setFormData({...formData, role: val, department_code: '', school_code: ''})}
                  options={[
              ...
              {formData.role === 'staff' && (
                <div className="animate-in slide-in-from-top-2">
                  <ShadcnSelect
                    ...
                    onChange={(val) => setFormData({...formData, department_code: val})}
                    options={adminDepts.map(d => ({ v: d.code, l: `${d.name} (${d.code})` }))}
              ...
              {formData.role === 'hod' && (
                <div className="animate-in slide-in-from-top-2">
                  <ShadcnSelect
                    value={formData.department_code}
                    onChange={(val) => setFormData({...formData, department_code: val})}
                    options={academicDepts.map(d => ({ v: d.code, l: `${d.name} (${d.code})` }))}
            ...
            <div className="flex gap-4 pt-6">
              <button type="button" onClick={onClose} className="flex-1 px-6 py-4 border border-slate-200 rounded-2xl font-black text-[10px] text-slate-400 hover:bg-slate-50 uppercase tracking-[0.2em] transition-all">
                Cancel
              </button>
              <button 
                type="submit" 
