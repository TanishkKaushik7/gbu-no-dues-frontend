...
const HODDashboard = () => {
  ...
  // --- 1. Fetch Department-Specific Pending Applications ---
  const fetchApplications = useCallback(async () => {
    setIsLoading(true);
    try {
      const authToken = localStorage.getItem('token');
      
      // Using the same standardized approval endpoint
  ...
  // --- 3. Handle HOD Approval/Rejection ---
  const handleHODAction = async (application, action, remarksIn) => {
    ...
    if (!stageId) return setActionError('No actionable stage found for Departmental clearance.');
  
    // HODs usually have department_id in their profile
    ...
    try {
      ...
      // Update local state: remove processed application
      setApplications(prev => prev.filter(app => app.id !== application.id));
      setSelectedApplication(null); 
    } catch (err) {
      const msg = err.response?.data?.detail || 'Error processing HOD action';
      setActionError(msg);
    } finally {
    ...
    <div className="flex h-screen bg-gray-100 font-sans overflow-hidden">
          ...
          >
            ...
            <motion.div variants={itemVariants}>
              ...
              <p className="text-sm sm:text-base text-gray-600 mt-1 sm:mt-2 mb-2 sm:mb-4">
                Verify student records and grant departmental clearance.
              </p>
            </motion.div>
      ...
      {selectedApplication && (
        <ApplicationActionModal 
          ...
          onAction={handleHODAction} 
          actionLoading={actionLoading} 
          actionError={actionError}
          userSchoolName={user?.department_name || 'Department Office'}
        />
      )}
    </div>
  );
