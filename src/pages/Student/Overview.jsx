...
import {
  ...
  FiDownload, FiShield, FiClock, FiArrowRight
} from 'react-icons/fi';

const Overview = ({ user, formData, stepStatuses = [], setActive, applicationId, application }) => {
  ...
  const isCompleted = application?.status === 'completed' || 
                      (stepStatuses.length > 0 && stepStatuses.every(s => s.status === 'completed'));
  ...
  if (stepStatuses.length > 0) {
    ...
    const completedSteps = stepStatuses.filter(s => 
      ['completed', 'approved', 'done'].includes((s.status || '').toLowerCase())
    ).length;
  ...
  let progressPercent = isCompleted ? 100 : (backendProgress ?? computedProgress ?? 0);
    ...
    <div className="space-y-6 lg:space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-6 flex items-center gap-6 min-w-[280px]">
          <div className="relative w-20 h-20 flex-shrink-0">
            <svg className="w-full h-full transform -rotate-90">
              <circle cx="40" cy="40" r="36" stroke="currentColor" strokeWidth="8" fill="transparent" className="text-slate-700/50" />
              <circle 
          ...
          <div className="flex flex-col gap-1">
            ...
            {isFullyCleared ? (
              <div className="flex flex-col items-start gap-2">
                <span className="text-xs font-black text-emerald-400 uppercase bg-emerald-500/10 px-2 py-1 rounded">Approved</span>
                <button
                ...
                >
                  {downloading ? <FiClock className="animate-spin" /> : <FiDownload size={14} />}
                  PDF
                </button>
              </div>
            ) : (
              <div>
                ...
                <p className="text-[10px] text-slate-400 mt-0.5">Pending Approvals</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <InfoCard 
          ...
          value={formData.schoolName || user?.student?.school_name || user?.school_name} 
          iconBg="bg-amber-50" 
        ...
        <InfoCard 
          icon={FiActivity} 
          label="Current Stage" 
          value={isFullyCleared ? "Completed" : (stepStatuses.find(s => s.status === 'pending' || s.status === 'in_progress')?.name || "Processing")}
...
const InfoCard = ({ 
  ...
  <div className={`${bgColor} p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col gap-4 transition-all hover:shadow-md`}>
    <div className={`w-10 h-10 ${iconBg} ${iconColor} rounded-xl flex items-center justify-center shadow-sm border border-slate-50/50`}>
      <Icon size={18} />
    </div>
    <div>
  ...
  return (
    ...
    >
      <div className="flex gap-5 items-center">
        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-300 ${currentTheme.iconBg} ${currentTheme.hoverIconBg}`}>
          <Icon size={24} />
        </div>
        <div>
          <h4 className="text-lg font-black text-slate-900 uppercase tracking-tight">{title}</h4>
          <p className="text-xs text-slate-400 font-medium mt-1 group-hover:text-slate-600 transition-colors">{desc}</p>
        </div>
      </div>
