...
const StudentDashboard = () => {
  ...
  const getActiveFromPath = (pathname) => {
    ...
    if (segment === "student" || !segment) return "dashboard";
    if (segment === "application") return "form";
    return segment;
  };
  ...
  const fetchApplicationStatus = useCallback(async () => {
    ...
    try {
      ...
      const rejectedFlag =
        body?.flags?.is_rejected || body?.application?.status === "rejected";
      setIsRejected(rejectedFlag);

      if (rejectedFlag) {
        setRejectionDetails(
          body?.rejection_details || {
            role: "Authority",
      ...
      const completedFlag = !!(
        body?.flags?.is_completed ||
        body?.application?.status?.toLowerCase() === "completed"
      );
      ...
      const mapStageToStatus = (stage, body) => {
        ...
        if (
          body?.application &&
          Number(body.application.current_stage_order) ===
            Number(stage.sequence_order)
        ) {
      ...
      let deptSeq =
        body.departments || body.department_sequence || DEFAULT_DEPT_SEQUENCE;
      ...
      const mappedStatuses = deptSeq.map((d) => {
        const stage = stages.find(
          (s) =>
            s.verifier_role?.toLowerCase() === d.name?.toLowerCase() ||
            s.sequence_order === d.sequence_order,
        );
  ...
  useEffect(() => {
    const nextActive = getActiveFromPath(location.pathname);
  ...
  const navigateToSection = (sectionId) => {
    const nextPath = studentPagePaths[sectionId] || `/student/${sectionId}`;
    setActive(sectionId);
    navigate(nextPath);
    ...
    if (type === "file") {
      ...
      try {
        ...
        if (res.data.path) {
          setFormData((prev) => ({
            ...prev,
            proof_document_url: res.data.path,
          }));
        }
      } catch (err) {
        setFormErrors((prev) => ({
          ...prev,
          [name]: "Upload failed. Ensure file is PDF < 5MB.",
        }));
      } finally {
  ...
  const handleSave = async (childPayload = null) => {
    ...
    try {
      ...
      const errorMsg =
        err.response?.data?.detail || err.message || "Error saving application";
      ...
      <aside className="hidden lg:flex flex-col w-72 xl:w-80 bg-slate-900 text-white p-8 xl:p-10 justify-between relative z-20 shadow-2xl">
        <div>
          <div className="flex items-center gap-4 mb-12 xl:mb-16 px-2">
            ...
            <h1 className="text-[12px] xl:text-sm font-black uppercase tracking-[0.25em]">
              Student Portal
            </h1>
          </div>
        ...
        <div
          className="pt-8 border-t border-slate-800/50 space-y-4 cursor-pointer"
        ...
        >
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <span className="text-[10px] xl:text-[11px] font-black uppercase tracking-[0.2em]">
                Logout
              </span>
            </div>
          </button>
        ...
        <header className="min-h-[72px] lg:h-24 border-b border-slate-50 flex items-center justify-between px-6 md:px-10 lg:px-14 bg-white/80 backdrop-blur-md sticky top-0 z-30">
          <div className="flex items-center gap-3 lg:gap-4">
            <button
              ...
              className="lg:hidden p-2 -ml-2 text-slate-900 hover:bg-slate-50 rounded-lg transition-colors"
            >
              <FiMenu size={24} />
            </button>
              ...
              >
                ...
                {active === "form" && (
                  <MyApplications
                    ...
                    hasSubmittedApplication={!!applicationId}
          ...
          <>
            ...
            >
              ...
              <div className="border-t border-slate-800/50 pt-6 mt-4">
                ...
                <button
                  onClick={handleLogout}
                  className="w-full flex items-center gap-4 p-3 text-slate-400 hover:text-rose-400 font-black uppercase tracking-widest transition-all bg-white/5 rounded-xl"
                >
