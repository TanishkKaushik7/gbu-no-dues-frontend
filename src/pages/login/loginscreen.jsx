...
const Button = React.forwardRef(
  ({ className, variant = "default", size = "default", ...props }, ref) => {
    ...
    const variants = {
      ...
      outline:
        "bg-background hover:bg-accent hover:text-accent-foreground border border-gray-200",
...
const LoginScreen = ({
  ...
  const getLoginErrorMessage = (err) => {
    ...
    return err?.message || "Access denied. Please try again.";
  };

  const handleRefreshVerification = (clearError = true) => {
    setTurnstileToken("");
    if (clearError) {
      setError("");
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (user) {
        ...
        if (rawRole === "staff") {
          switch (deptId) {
            ...
            case null:
              targetPath = userName.includes("office")
                ? "/office/dashboard"
                : "/school/dashboard";
              break;
      ...
      // Reset verification after failed login but keep the error visible.
      handleRefreshVerification(false);
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="w-full max-w-[400px] lg:max-w-4xl grid grid-cols-1 lg:grid-cols-10 bg-white rounded-[2rem] sm:rounded-[2.5rem] shadow-2xl shadow-blue-900/10 border border-slate-100 overflow-hidden relative z-10 my-12 lg:my-0">
        <div className="lg:col-span-4 bg-slate-900 p-8 sm:p-10 flex flex-col justify-between text-white relative overflow-hidden min-h-[200px] lg:min-h-auto">
          ...
          <div className="relative z-10">
            ...
            <p className="text-slate-400 text-xs mt-4 sm:mt-6 font-bold uppercase tracking-[0.2em]">
              {systemName}
            </p>
          </div>
        </div>
            ...
            <AnimatePresence mode="wait">
              {error && (
                <motion.div
                  ...
                  exit={{ opacity: 0 }}
                  className="mb-6 p-4 bg-red-50 text-red-600 border border-red-100 rounded-2xl text-[10px] font-bold uppercase tracking-wider flex items-center gap-3"
                >
