...
// Switched from react-icons/fi to lucide-react to resolve the build error
import { 
  Calendar, Eye, RefreshCw, Clock, MapPin, Search, Filter, Loader, AlertTriangle,
  ChevronDown, Check
} from 'lucide-react';

// Restored your actual auth context so the table stops hiding your real backend data
  ...
  if (['inprogress', 'in_progress', 'pending'].includes(key)) {
    return (
      <span className="bg-yellow-100 text-yellow-800 text-xs font-medium px-2.5 py-0.5 rounded flex items-center w-fit">
        <Clock className="mr-1 w-3 h-3" /> {s}
      </span>
    );
  }
...
const ApplicationsTable = ({ applications = [], isLoading, onView, onSearch, onRefresh, isViewLoading }) => {
  ...
  const filteredApps = useMemo(() => {
    ...
    let result = applications.filter(app => {
      // Make sure the role array matches your actual backend roles
    ...
    >
      <div className="flex flex-col md:flex-row md:items-center md:justify-between border-b pb-4 mb-4 gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
          ...
          <div className="flex items-center border border-gray-300 rounded-lg px-3 py-2 w-full md:w-64 bg-gray-50/50">
            <Search className="text-gray-500 mr-2 flex-shrink-0 w-4 h-4" />
      ...
      {isLoading ? (
        <div className="text-center py-12 text-gray-500 flex flex-col items-center">
          <RefreshCw className="animate-spin w-6 h-6 mb-3 text-indigo-500" />
            ...
            <table className="min-w-full divide-y divide-gray-200 table-fixed">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="w-32 px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Enrollment No</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {paginatedApplications.map((app) => (
                  <tr key={app.id} className={`transition-colors duration-200 ${app.is_overdue ? 'bg-red-50/40 hover:bg-red-50/70 border-l-4 border-l-red-500' : 'hover:bg-indigo-50/50'}`}>
                    ...
                    <td className="px-4 py-3 text-gray-900 font-semibold whitespace-nowrap overflow-hidden text-ellipsis">
                      {app.rollNo || '—'}
                    </td>
                    
                    <td className="px-4 py-3 text-gray-500 text-sm whitespace-nowrap overflow-hidden text-ellipsis">
                      {app.enrollment || '—'}
                    </td>
                    ...
                    <td className="px-4 py-3 text-gray-700 text-sm overflow-hidden">
                       <div className="flex items-center text-gray-500 max-w-[180px]" title={app.current_location}>
                    ...
                    <td className="px-4 py-3 text-gray-700 whitespace-nowrap">
                      <div className='flex flex-col'>
                          <div className='flex items-center text-sm'>
                              <Calendar className='mr-1 w-3 h-3 text-gray-400 flex-shrink-0' />
                              {formatDate(app.date)}
                          </div>
                          {app.is_overdue && (
                              <span className="mt-1 inline-flex items-center gap-1 text-[10px] font-bold text-red-600 bg-red-100 px-1.5 py-0.5 rounded w-fit">
                                <AlertTriangle className="w-3 h-3 flex-shrink-0" /> {app.days_pending}d
                              </span>
                          )}
                      </div>
                    </td>
